...
void System ::initialize(string state)
{ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream couta("../OUTPUT/" + state + "/acceptance.dat"); // Set the heading line in file ../OUTPUT/"+state+"/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/input." + state); // Start reading ../INPUT/input.dat (or)
  ofstream coutf;
  coutf.open("../OUTPUT/" + state + "/output.dat");
  string property;
  double delta;
  while (!input.eof()){
    input >> property;
...
void System ::initialize_properties(string state){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      {
        ofstream coutp("../OUTPUT/" + state + "/potential_energy.dat");
      ...
      {
        ofstream coutk("../OUTPUT/" + state + "/kinetic_energy.dat");
      ...
      {
        ofstream coutt("../OUTPUT/" + state + "/total_energy.dat");
      ...
      {
        ofstream coutte("../OUTPUT/" + state + "/temperature.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + state + "/pressure.dat");
      ...
      {
        ofstream coutgr("../OUTPUT/" + state + "/gofr.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + state + "/magnetization.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + state + "/specific_heat.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + state + "/susceptibility.dat");
      ...
      {
        ofstream coutf;
        coutf.open("../OUTPUT/" + state + "/output.dat", ios::app);
...
void System ::finalize(string state)
{
  this->write_configuration(state);
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/" + state + "/output.dat", ios::app);
...
void System ::write_configuration(string state){
  ofstream coutf;
  if (_sim_type < 2){
    coutf.open("../OUTPUT/"+state+"/CONFIG/config.xyz");
    if (coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for (int i = 0; i < _npart; i++){
    ...
    coutf.close();
    this->write_velocities(state);
  } else {
    coutf.open("../OUTPUT/"+state+"/CONFIG/config.spin");
...
void System ::write_XYZ(int nconf, string state){
  ofstream coutf;
  coutf.open("../OUTPUT/" +state+ "/CONFIG/config_" + to_string(nconf) + ".xyz");
  if (coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for (int i = 0; i < _npart; i++){
...
void System ::write_velocities(string state){
  ofstream coutf;
  coutf.open("../OUTPUT/"+state+"/CONFIG/velocities.out");
  if (coutf.is_open()){
    for (int i = 0; i < _npart; i++){
...
void System ::block_reset(int blk, string state)
...
void System ::averages(int blk, string state)
{
  ...
  if (_measure_penergy)
  {
    coutf.open("../OUTPUT/"+state+"/potential_energy.dat", ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT/"+state+"/kinetic_energy.dat", ios::app);
  ...
  if (_measure_tenergy)
  {
    coutf.open("../OUTPUT/"+state+"/total_energy.dat", ios::app);
  ...
  if (_measure_temp)
  {
    coutf.open("../OUTPUT/" + state + "/temperature.dat", ios::app);
  ...
  if (_measure_pressure){
    coutf.open("../OUTPUT/"+state+"/pressure.dat", ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT/"+state+"/acceptance.dat", ios::app);
