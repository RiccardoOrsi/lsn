...
void System ::initialize(string in, string out)
{ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream couta("../OUTPUT/" + out + "/acceptance.dat"); // Set the heading line in file ../OUTPUT/"+state+"/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/input." + in); // Start reading ../INPUT/input.dat (or)
  ofstream coutf;
  coutf.open("../OUTPUT/" + out + "/output.dat");
  string property;
  double delta;
  while (!input.eof()){
    input >> property;
...
void System ::initialize_velocities()
{
  ...
  { // If restart read previous velocities
    ifstream cinf;
  ...
  if (_sim_type == 0)
  {
    ...
    for (int i = 0; i < _npart; i++)
    { // _xold initialization for Verlet algorithm
...
void System ::initialize_properties(string out){ // Initialize data members used for measurement of properties
  ...
  _measure_temp = false;        //THIS LINE WAS MISSING FROM THE OG CODE, ignoring it causes "troubles" with _sim_type being 2 or 3
  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      {
        ofstream coutp("../OUTPUT/" + out + "/potential_energy.dat");
      ...
      {
        ofstream coutk("../OUTPUT/" + out + "/kinetic_energy.dat");
      ...
      {
        ofstream coutt("../OUTPUT/" + out + "/total_energy.dat");
      ...
      {
        ofstream coutte("../OUTPUT/" + out + "/temperature.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + out + "/pressure.dat");
      ...
      {
        ofstream coutgr("../OUTPUT/" + out + "/gofr.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + out + "/magnetization.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + out + "/specific_heat.dat");
      ...
      {
        ofstream coutpr("../OUTPUT/" + out + "/susceptibility.dat");
      ...
      {
        ofstream coutf;
        coutf.open("../OUTPUT/" + out + "/output.dat", ios::app);
...
void System ::finalize(string out)
{
  this->write_configuration(out);
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/" + out + "/output.dat", ios::app);
...
// Write current configuration as a .xyz file in directory ../OUTPUT/"+state+"/CONFIG/
void System ::write_configuration(string out){
  ofstream coutf;
  if (_sim_type < 2){
    coutf.open("../OUTPUT/"+out+"/CONFIG/config.xyz");
    if (coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for (int i = 0; i < _npart; i++){
    ...
    coutf.close();
    this->write_velocities(out);
  } else {
    coutf.open("../OUTPUT/"+out+"/CONFIG/config.spin");
...
// Write configuration nconf as a .xyz file in directory ../OUTPUT/"+state+"/CONFIG/
void System ::write_XYZ(int nconf, string out){
  ofstream coutf;
  coutf.open("../OUTPUT/" +out+ "/CONFIG/config_" + to_string(nconf) + ".xyz");
  if (coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for (int i = 0; i < _npart; i++){
...
void System ::write_velocities(string out){
  ofstream coutf;
  coutf.open("../OUTPUT/"+out+"/CONFIG/velocities.out");
  if (coutf.is_open()){
    for (int i = 0; i < _npart; i++){
...
// Read configuration from a .xyz file in directory ../OUTPUT/"+state+"/CONFIG/
...
void System ::block_reset(int blk, string out)
...
void System ::measure(){ // Measure properties
  ...
  double tenergy_temp = 0.0;      // temporary accumulator for total energy (kinetic+potential)
...
if (_measure_gofr){
  ...
  if (_measure_pressure){
    _measurement(_index_pressure) = _ptail + _rho * temperature_temp + (48. / (3. * _volume)) * pressure_temp;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    for (int i=0; i<_npart; i++){
...
void System ::averages(int blk, string out)
{
  ...
  if (_measure_penergy)
  {
    coutf.open("../OUTPUT/"+out+"/potential_energy.dat", ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT/"+out+"/kinetic_energy.dat", ios::app);
  ...
  if (_measure_tenergy)
  {
    coutf.open("../OUTPUT/"+out+"/total_energy.dat", ios::app);
  ...
  if (_measure_temp)
  {
    coutf.open("../OUTPUT/" + out + "/temperature.dat", ios::app);
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure){
    coutf.open("../OUTPUT/"+out+"/pressure.dat", ios::app);
 ...
 if (_measure_gofr and (blk == _nblocks)){
    coutf.open("../OUTPUT/" +out+ "/gofr.dat",ios::app);
    for (int i=_index_gofr;i<_index_gofr+_n_bins;i++)  {
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    coutf.open("../OUTPUT/"+out+"/magnetization.dat",ios::app);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    coutf.open("../OUTPUT/"+out+"/specific_heat.dat",ios::app);
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/"+out+"/susceptibility.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT/"+out+"/acceptance.dat", ios::app);
...
void System :: print_es6(string out, int blk){

  ofstream coutre;
  if (_measure_tenergy){
    coutre.open(out+"/total_energy.out",ios::app);
  ...
  if (_measure_cv){
    coutre.open(out+"/heat_capacity.out",ios::app);
  ...
  if (_measure_chi){
    coutre.open(out+"/susceptibility.out",ios::app);
  ...
  if (_measure_magnet){
    coutre.open(out+"/magnetization.out",ios::app);
